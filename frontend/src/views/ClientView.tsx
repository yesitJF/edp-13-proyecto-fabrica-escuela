...
import { IcoDash, IcoCuentas, IcoTransfer, IcoPagos, IcoPrestamos, IcoInversiones, IcoExtractos, IcoAlert, IcoChevron, IcoInfo, IcoShield, IcoStar } from "../shared/icons";
import { generarNumeroCuenta } from "../shared/helpers";
...
const CUENTA_INFO = {
  ahorros: {
    ...
    descripcion: "Ideal para guardar tu dinero y ganar rendimientos. Sin cuota de manejo los primeros 6 meses.",
    beneficios: ["Sin cuota de manejo por 6 meses", "Rendimientos del 4.2% E.A.", "Tarjeta débito virtual incluida", "Retiros ilimitados en cajeros BX"],
    badge: "Más popular",
  },
  corriente: {
    titulo: "Cuenta Corriente",
    descripcion: "Perfecta para movimientos frecuentes y transacciones comerciales sin límite de operaciones.",
    beneficios: ["Transacciones ilimitadas", "Chequera digital incluida", "Acceso a sobregiro aprobado", "Integración con pasarelas de pago"],
    badge: "Para negocios",
  },
};
...
export default function ClientView() {
  ...
  function handleContinue() {
    if (!tipoCuenta) { setError("Selecciona un tipo de cuenta para continuar."); return; }
    setError("");
    setStep("confirm");
  }

  function handleConfirm() {
    setNumeroCuenta(generarNumeroCuenta(tipoCuenta));
    setStep("success");
  }
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="px-8 py-5 flex items-center justify-between sticky top-0 z-10" style={{ background: "rgba(15,10,30,0.92)", borderBottom: "1px solid rgba(139,92,246,0.1)", backdropFilter: "blur(8px)" }}>
          <div className="flex items-center gap-2 text-sm" style={{ color: "#7c6fa0" }}>
            <span>Mis Cuentas</span><IcoChevron /><span style={{ color: "#c4b5fd" }}>Abrir Nueva Cuenta</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <span className="px-3 py-1.5 rounded-lg text-xs" style={{ background: "rgba(109,40,217,0.15)", color: "#c4b5fd", border: "1px solid rgba(139,92,246,0.2)" }}>HU-02-01-01</span>
          </div>
        </div>

        <div className="px-8 py-6 max-w-4xl">
          <div className="mb-8">
            <span className="text-xs font-semibold uppercase tracking-wider px-2 py-0.5 rounded mb-2 inline-block" style={{ background: "rgba(168,85,247,0.15)", color: "#a78bfa", fontFamily: "Instrument Sans,sans-serif" }}>Nueva Cuenta</span>
            <h1 className="text-2xl font-bold mb-1" style={{ color: "#f1eeff", fontFamily: "Instrument Sans,sans-serif", letterSpacing: "-0.3px" }}>Abre tu cuenta financiera</h1>
            <p className="text-sm" style={{ color: "#7c6fa0" }}>Selecciona el tipo de cuenta que mejor se adapta a tus necesidades.</p>
          </div>

          {/* Stepper */}
          <div className="flex items-center gap-0 mb-8">
            {STEPS.map((s, i) => {
              const done = (step === "confirm" && s.key === "select") || step === "success";
              ...
              return (
                <div key={s.key} className="flex items-center">
                  ...
                  {i < 2 && <div className="w-12 h-px mx-3" style={{ background: done ? "rgba(16,185,129,0.4)" : "rgba(139,92,246,0.15)" }} />}
                </div>
              );
            })}
          </div>
          ...
          {step === "select" && (
            <div className="flex flex-col gap-5">
              <div className="flex items-center gap-4 rounded-2xl px-5 py-4" style={{ background: "#130d24", border: "1px solid rgba(139,92,246,0.13)" }}>
                <div className="flex items-center justify-center w-10 h-10 rounded-full font-bold" style={{ background: "linear-gradient(135deg,rgba(109,40,217,0.4),rgba(168,85,247,0.3))", color: "#c4b5fd", fontFamily: "Instrument Sans,sans-serif" }}>{CLIENTE_ACTIVO.iniciales}</div>
                <div className="flex-1">
                  <div className="text-sm font-semibold" style={{ color: "#f1eeff" }}>{CLIENTE_ACTIVO.nombre}</div>
                  <div className="text-xs" style={{ color: "#7c6fa0" }}>CC {CLIENTE_ACTIVO.cedula} · {CLIENTE_ACTIVO.email}</div>
                </div>
                <div className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold" style={{ background: "rgba(16,185,129,0.12)", color: "#34d399", border: "1px solid rgba(16,185,129,0.25)" }}>
                  <IcoShield /> Verificado
                </div>
                <div className="text-xs px-3 py-1 rounded-full" style={{ background: "rgba(139,92,246,0.1)", color: "#a78bfa", border: "1px solid rgba(139,92,246,0.2)" }}>ID: {CLIENTE_ACTIVO.id}</div>
              </div>
              ...
              <div className="grid gap-4" style={{ gridTemplateColumns: "1fr 1fr" }}>
                {(["ahorros", "corriente"] as const).map((tipo) => {
                  ...
                  return (
                    <button key={tipo} onClick={() => { setTipoCuenta(tipo); setError(""); }} className="relative text-left rounded-2xl p-6 transition-all duration-200 w-full" style={{ background: selected ? "linear-gradient(135deg,rgba(109,40,217,0.22),rgba(168,85,247,0.12))" : "#130d24", border: selected ? "2px solid rgba(139,92,246,0.55)" : "2px solid rgba(139,92,246,0.13)", boxShadow: selected ? "0 0 0 4px rgba(109,40,217,0.1)" : "none" }}>
                      <div className="absolute top-4 right-4 text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: selected ? "rgba(168,85,247,0.25)" : "rgba(139,92,246,0.1)", color: selected ? "#c4b5fd" : "#7c6fa0", border: `1px solid ${selected ? "rgba(139,92,246,0.4)" : "rgba(139,92,246,0.15)"}` }}>{ci.badge}</div>
                      <div className="flex items-center justify-center w-12 h-12 rounded-xl mb-4" style={{ background: selected ? "rgba(109,40,217,0.3)" : "rgba(139,92,246,0.1)" }}>
                        <span style={{ color: selected ? "#a78bfa" : "#7c6fa0", transform: "scale(1.4)", display: "block" }}><IcoCuentas /></span>
                      </div>
                      <div className="pr-16">
                        <div className="text-base font-bold mb-1" style={{ color: selected ? "#f1eeff" : "#c4b5fd", fontFamily: "Instrument Sans,sans-serif" }}>{ci.titulo}</div>
                        <div className="text-xs mb-4" style={{ color: "#7c6fa0", lineHeight: "1.5" }}>{ci.descripcion}</div>
                        <ul className="flex flex-col gap-2">
                          {ci.beneficios.map((b) => (
                            <li key={b} className="flex items-start gap-2 text-xs" style={{ color: selected ? "#c4b5fd" : "#7c6fa0" }}>
                              <span className="mt-0.5 shrink-0" style={{ color: selected ? "#a78bfa" : "#4c1d95" }}><IcoStar /></span>{b}
                            </li>
                          ))}
                        </ul>
                      </div>
                      <div className="absolute bottom-4 right-4 w-5 h-5 rounded-full flex items-center justify-center" style={{ background: selected ? "linear-gradient(135deg,#6d28d9,#a855f7)" : "rgba(139,92,246,0.12)", border: selected ? "none" : "2px solid rgba(139,92,246,0.25)" }}>
              ...
              <div className="flex items-start gap-3 rounded-xl px-4 py-3.5" style={{ background: "rgba(139,92,246,0.06)", border: "1px solid rgba(139,92,246,0.15)" }}>
                <span style={{ color: "#a78bfa", flexShrink: 0, marginTop: "1px" }}><IcoInfo /></span>
                <p className="text-xs" style={{ color: "#7c6fa0", lineHeight: "1.6" }}>El <strong style={{ color: "#a78bfa" }}>saldo inicial será $0</strong>. El número de cuenta se genera automáticamente.</p>
              </div>
                ...
                <div className="px-6 py-5" style={{ background: "#130d24" }}>
                  {[
                    ...
                    <div key={row.label} className="flex items-center justify-between py-2" style={{ borderBottom: "1px solid rgba(139,92,246,0.08)" }}>
                      ...
                      <span className="text-sm font-medium" style={{ color: (row as { green?: boolean }).green ? "#34d399" : (row as { highlight?: boolean }).highlight ? "#c4b5fd" : (row as { muted?: boolean }).muted ? "#7c6fa0" : "#f1eeff" }}>{row.value}</span>
                    </div>
                  ))}
                </div>
